<template>
    <div class="add_addres">
        <div class="add_list">
          <ul>
            <li @click="fnFoldShow()">
              <span class="type fontSize30">发票类型</span><span class="company fontSize30">{{cardText}}</span><span class="iconfont icon-xiangyou"></span>
            </li>
            <li>
              <span class="type fontSize30">发票抬头</span><input type="text"  placeholder="请填写发票抬头" ref="title">
            </li>
            <li v-show="duty === 0">
              <span class="type fontSize30">纳税人识别号</span><input type="text"  placeholder="仅限字母和数字" ref="duty">
            </li>
          </ul>
        </div>
        <div class="sure_btn">
          <div class="delete fontSize36" @click="del">删除</div>
          <div class="sure fontSize36" @click="fnSure">确定</div>
        </div>
      ...
      fnSure(){
        ...
        if(this.duty === 0){
          ...
          let obj = {
            ...
            tax_codeField:this.duty == 0 ? duty.value :'',
          }
//          console.log(obj)
          this.submit(obj);
      },
      ...
      submit: async function (obj) {
       ...
       let res = ''
       //if(obj.typeField == 0){
       //   res = await this.$root.http.post('invoice/addProfessional', params,this)
       //}else{
       //   res = await this.$root.http.post('invoice/addCommon', params,this)
       //}
       res = await this.$root.http.post('invoice/addCommon', params,this)
