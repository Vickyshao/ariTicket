...
<script>
  ...
  export default {
    ...
    computed:{
      ...mapState({
        airline:state =>state.list.filterList.airline,
        orgAirport:state =>state.list.filterList.orgAirport,
        dstAirport:state =>state.list.filterList.dstAirport,
        craftType:state =>state.list.filterList.craftType,
        queryAllCityList:state =>state.list.queryAllCityList,
        showCityList:state =>state.list.showCityList,
...
//        console.log('机型：'+craftType);
        ...
        if(orgAirport.length===0&&dstAirport.length===0&&time.length===0&&airline.length===0&&craftType.length===0){
          this.$store.commit('UPDATEASALL');   //重新加载全部数据
...
//              console.log(timeArr);
        ...
        //更新 最终数组
        this.$store.commit('FILTER_FLIGHTS',newsFlyList);
