...
<script>
  import FilterPoput from './returnFilterPopup.vue'
  import { mapState, mapMutations } from 'vuex'
export default {
  data(){
    return{
      isTimeSelected:false,     //时间排序
      fromEarlyToLate:false,    //时间：是否是从早到晚
...
//    VFilterpoput,
  ...
  computed:{
    ...mapState({
      isShowLoading:state =>state.returnList.isShowLoading,
      showCityList:state =>state.returnList.showCityList,
      isHasFilter:state =>state.returnList.isHasFilter,
    })
  },
  methods:{
    ...
    timeSort:function () {
      ...
      if(this.fromEarlyToLate){
        ...
        this.$store.commit('FILTER_FLIGHTS_RETURN',arr);
      }
      else{
