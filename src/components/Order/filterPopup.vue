  ...
  <section class="filterPopup">
    ...
    <section :class="{show:isShow}" class="filterList">
      <div class="topOpt fontSize30">
        <span class="ml30 fontSize30" @click="filterCancel">取消</span>
        <span :class="{gray:isAnyJC&&isAnySJ&&isAnyJX}" class="clearSelected fontSize30" @click="clearSelected">清空已选</span>
        <span class="mr30 fontSize30" @click="selectedOk">确定</span>
      </div>
...
<script>
  ...
  export default {
    ...
    computed:{
      ...mapState({
        airline:state =>state.changeFlyList.filterList.airline,
        orgAirport:state =>state.changeFlyList.filterList.orgAirport,
        dstAirport:state =>state.changeFlyList.filterList.dstAirport,
        craftType:state =>state.changeFlyList.filterList.craftType,
        queryAllCityList:state =>state.changeFlyList.queryAllCityList,
        showCityList:state =>state.changeFlyList.showCityList,
        // departDate:state =>state.changeFlyList.departDateList.departDate,
      }),
      //机场
      isAnyJC:function () {
        var isany=true;
...
//              console.log(timeArr);
          ...
          //最终筛选结果
          if(isOrgAirport && isDstAirport && isTime && isCraftType){
            return true;
          }
          else{
            return false;
        ...
        //更新 最终数组
        this.$store.commit('CHANGE_FILTER_FLIGHTS',newsFlyList);
        if(this.showCityList.length===0){
          this.$store.commit('ERROR_SHOW','没有查询到符号条件的航班');
          console.log('为啥会走这呢');
        }
        else{
          this.$store.commit('ERROR_HIDDEN');       //错误页隐藏
